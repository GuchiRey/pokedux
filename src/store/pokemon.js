  ...
  actions: {
    ...
    filterPokemons(filters) {
      ...
      this.pokemonsFiltered.forEach((p, index) => {
        if(index <= filters.page * 16 && index >= filters.page * 16 - 16) {
    ...
    clearFilters(){
      ...
      this.pokemonsRender = this.pokemons
      this.searchStatus.count = 0
      this.searchStatus.active = false
    },
    deletePokemons(){
      this.pokemonsRender = this.pokemonsRender.filter((pokemon) => {
    ...
    deletePokemon(pokemon){
      this.pokemonsRender = this.pokemonsRender.filter((p) => {
    ...
    createPokemon(pokemon){
      ...
      this.pokemonsRender.unshift(pokemon);
    }
  }
});